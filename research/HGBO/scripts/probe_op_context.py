"""Test TBE compile with OpContext."""
import textwrap, paramiko

script = textwrap.dedent(r'''
import json, traceback, struct, sys
...
x = {"shape": (IH,IW,IC), "dtype": "float16", "format": "ND", "ori_shape": (IH,IW,IC), "ori_format": "ND", "param_type": "required"}
y = {"shape": (OH,OW,OC), "dtype": "float16", "format": "ND", "ori_shape": (OH,OW,OC), "ori_format": "ND", "param_type": "required"}
...
try:
    ...
    out["compile"] = traceback.format_exc()[-1500:]
...
sftp = ssh.open_sftp()
with sftp.open("/tmp/probe_opctx.py","w") as f: f.write(script.replace("\r\n","\n"))
sftp.close()
cmd = ("source /usr/local/Ascend/ascend-toolkit/set_env.sh && "
       ...
       "python3 /tmp/probe_opctx.py 2>&1")
