"""TBE compile with build_config() context."""
import paramiko
...
python3 <<'PY'
import json, traceback, struct, sys, glob, shutil, os
...
try:
    ...
    out["compile"] = traceback.format_exc()[-5000:]

hits = glob.glob("/tmp/**/video_pre_fuse_custom*.o", recursive=True) + glob.glob("/root/**/video_pre_fuse_custom*.o", recursive=True)
out["o_files"] = [h for h in hits if "CMakeFiles" not in h][:20]
hits2 = glob.glob("/tmp/**/video_pre_fuse_custom*.json", recursive=True)
out["json_files"] = hits2[:20]
...
sftp = ssh.open_sftp()
with sftp.open("/tmp/probe_buildcfg.sh", "w") as f:
    f.write(script.replace("\r\n", "\n"))
sftp.close()
_, stdout, _ = ssh.exec_command("/bin/bash /tmp/probe_buildcfg.sh 2>&1", timeout=900)
