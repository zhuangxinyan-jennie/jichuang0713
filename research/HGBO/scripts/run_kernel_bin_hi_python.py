...
cmd = (
    ...
    f"cd {base} && bash {gen_sh} src/video_pre_fuse_custom.py bin/video_pre_fuse_custom 2>&1"
)
_, stdout, _ = ssh.exec_command(f"/bin/bash -lc {repr(cmd)}", timeout=600)
print(stdout.read().decode()[-4000:])
_, stdout, _ = ssh.exec_command(f"find {base}/bin -type f 2>/dev/null | head -10", timeout=15)
