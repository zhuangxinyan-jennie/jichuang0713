...
trials = []
for args in [
    ("VideoPreFuseCustom", [xd], [yd], 0, [], [], []),
    ("VideoPreFuseCustom", [xd], [yd], 0, None, None, None),
    ("VideoPreFuseCustom", [xd], [yd], 0, 0, 0, 0),
]:
    try:
        h,r = acl.op.create_handle(*args)
        e = acl.op.execute(h, [xb], [yb], None, stream)
        acl.rt.synchronize_stream(stream)
        trials.append({"args": str(args[:4]), "create": int(r), "exec": int(e)})
        acl.op.destroy_handle(h)
    except Exception as ex:
        trials.append({"args": str(args[:4]), "err": str(ex)})
...
sftp = ssh.open_sftp()
with sftp.open("/tmp/probe_acl4.py","w") as f: f.write(script.replace("\r\n","\n"))
sftp.close()
