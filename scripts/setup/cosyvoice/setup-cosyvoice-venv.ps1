...
$Repo = Join-Path $ProjectRoot "archive\tts-local\CosyVoice"

$Venv = Join-Path $Repo ".venv-clean"

$WinReq = Join-Path $PSScriptRoot "cosyvoice-requirements-windows.txt"
...
Write-Host "Ref: archive/cozy_ref (read-only reference, not install path)`n"
