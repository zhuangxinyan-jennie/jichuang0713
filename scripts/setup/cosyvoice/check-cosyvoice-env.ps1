...
$LocalTts = Join-Path $ProjectRoot "archive\tts-local"
$TtsRoot = Join-Path $ProjectRoot "cosyvoice_live_release"
$CosyRepo = if ($env:COSYVOICE_REPO) { $env:COSYVOICE_REPO } else { Join-Path $LocalTts "CosyVoice" }
$ModelDir = if ($env:COSYVOICE_MODEL_DIR) { $env:COSYVOICE_MODEL_DIR } else { Join-Path $LocalTts "CosyVoice2-0.5B" }
...
if (-not (Test-Path -LiteralPath $Python)) { throw "FAIL: Python not found. Run .\scripts\setup\cosyvoice\setup-cosyvoice-venv.ps1 first" }
