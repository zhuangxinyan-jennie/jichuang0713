...
namespace XiongdaImporter
{
    ...
    public sealed class XiongdaFaceBlendShapeDriver : MonoBehaviour
    {
        ...
        [Header("说话口型（A/O 交替，前端 TTS 播放时调用）")]
        public bool enableSpeechLipSync = true;

        [Tooltip("张嘴频率（Hz），约 8–12 像正常说话")]
        public float lipSyncHz = 9f;

        [Range(0f, 65f)]
        public float lipSyncWeightA = 44f;

        [Range(0f, 65f)]
        public float lipSyncWeightO = 34f;

        [Range(0f, 20f)]
        public float lipSyncWeightClosed = 4f;

        [Tooltip("振幅口型平滑速度（越大跟音量越紧）")]
        public float lipSyncAmplitudeSmoothSpeed = 18f;

        [Tooltip("振幅驱动时 O 口型相对 A 的比例")]
        [Range(0f, 1f)]
        public float lipSyncAmplitudeOScale = 0.42f;

        int _lipSyncActiveCount;
        bool _lipSyncUseAmplitudeDrive;
        float _lipSyncTargetAmplitude;
        float _lipSyncSmoothedAmplitude;
        ...
        /// <summary>
        /// WebGL / TTS：说话时在现有表情上叠加 A/O 口型张合（Start 与 Stop 成对调用）。
        /// </summary>
        public void StartSpeechLipSync()
        {
            if (!enableSpeechLipSync || !_configReady)
            {
                return;
            }
        ...
        public void StopSpeechLipSync()
        {
            if (_lipSyncActiveCount <= 0)
            {
                return;
            }
            ...
            if (_lipSyncActiveCount <= 0)
            {
                _lipSyncActiveCount = 0;
                _lipSyncUseAmplitudeDrive = false;
                _lipSyncTargetAmplitude = 0f;
                _lipSyncSmoothedAmplitude = 0f;
            }
        }

        /// <summary>
        /// WebGL：前端 Web Audio RMS 振幅 0–1，驱动 A/O 张嘴幅度。
        /// </summary>
        public void SetSpeechLipSyncLevel(string levelText)
        {
            if (!enableSpeechLipSync || !_configReady)
            {
                return;
            ...
            if (!float.TryParse(levelText, out float level))
            {
                return;
            }

            _lipSyncUseAmplitudeDrive = true;
            _lipSyncTargetAmplitude = Mathf.Clamp01(level);
        }

        bool IsSpeechLipSyncActive()
        {
            return enableSpeechLipSync && _lipSyncActiveCount > 0;
        }
        ...
        void LateUpdate()
        {
            ...
            {
                ApplySpeechLipSyncOverlay();
            }
        }

        void ApplySpeechLipSyncOverlay()
        {
            if (targetRenderer == null)
            {
                return;
            ...
            float fun = ReadCurrentLogical("fun");
            float happy = ReadCurrentLogical("happy");
            float cry = ReadCurrentLogical("cry");

            float mouthA;
            float mouthO;
            if (_lipSyncUseAmplitudeDrive)
            {
                _lipSyncSmoothedAmplitude = Mathf.Lerp(
                    _lipSyncSmoothedAmplitude,
                    _lipSyncTargetAmplitude,
                    Time.deltaTime * Mathf.Max(4f, lipSyncAmplitudeSmoothSpeed));
                float open = _lipSyncSmoothedAmplitude;
                mouthA = Mathf.Lerp(lipSyncWeightClosed, lipSyncWeightA, open);
                mouthO = Mathf.Lerp(lipSyncWeightClosed, lipSyncWeightO, open * lipSyncAmplitudeOScale);
            }
            else
            {
                float phase = Time.time * Mathf.Max(1f, lipSyncHz);
                float aWave = 0.5f + 0.5f * Mathf.Sin(phase * Mathf.PI * 2f);
                float oWave = 0.5f + 0.5f * Mathf.Sin(phase * Mathf.PI * 2f + 1.65f);
                mouthA = Mathf.Lerp(lipSyncWeightClosed, lipSyncWeightA, aWave);
                mouthO = Mathf.Lerp(lipSyncWeightClosed, lipSyncWeightO, oWave);
            }
        ...
        float ReadCurrentLogical(string shapeName)
        {
            if (!_logicalIndex.TryGetValue(shapeName, out int idx))
            {
                return 0f;
            ...
            return _current.TryGetValue(idx, out float v) ? v : 0f;
        }
