...
namespace SmartParkTerminal
{
    ...
    public sealed class UnityBridge : MonoBehaviour
    {
        ...
        /// <summary>
        /// WebGL：TTS 开始播放时调用，熊大嘴型 A/O 交替张合。
        /// </summary>
        public void StartSpeechLipSync()
        {
            ...
            if (face == null)
            {
                Debug.LogWarning("[UnityBridge] 未找到 XiongdaFaceBlendShapeDriver，无法 StartSpeechLipSync。");
                return;
            }

            face.StartSpeechLipSync();
        }

        /// <summary>
        /// WebGL：TTS 播放结束或打断时调用。
        /// </summary>
        public void StopSpeechLipSync()
        {
            ...
            face.StopSpeechLipSync();
        }

        /// <summary>
        /// WebGL：SendMessage 传入 "0"–"1" 字符串，表示当前语音 RMS 振幅。
        /// </summary>
        public void SetSpeechLipSyncLevel(string levelText)
        {
            ...
            face.SetSpeechLipSyncLevel(levelText);
        }
