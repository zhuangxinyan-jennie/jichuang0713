...
OUT_DIR="${OUT_DIR:-${BOARD_ROOT}/models_om}"
...
# CANN 8.0.0.alpha003 does not add the driver stub directory on WSL hosts.
# ATC needs this stub for offline compilation when the PC has no Ascend driver.
...
echo "[ATC] staging=${BUILD_DIR}"
echo "[ATC] compile_cores=${MAX_COMPILE_CORE_NUMBER} cache=${OP_CACHE_DIR}"
echo "[ATC] output_type=${OUTPUT_TYPE:-model_default}"
