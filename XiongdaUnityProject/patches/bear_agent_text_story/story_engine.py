"""
剧情互动状态机
只根据游客的 A/B/C 选择推进固定剧情，不调用LLM。
台词与 xiongda_app 中小剧场、theater_voice_manifest.json 一致。
"""
import re

CLIP_SPEECH = {
    "0": "俺今天是智慧乐园守护员，有三个小任务要完成。\n你来帮俺做选择，好不好？",
...
class StoryEngine:
    ...
    def _clip(self, clip_ids, *, story_finished=False):
        segments = []
        voice_ids = []
        for c in clip_ids:
            key = str(c).strip()
        ...
        if story_finished:
            ...
            payload["story_waiting_hint"] = {
                self.QUESTION_OPENING: "可以说：好一起完成任务 / 先听听规则，也可以说 A 或 B。",
