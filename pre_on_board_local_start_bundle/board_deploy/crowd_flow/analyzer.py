...
class CrowdConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if min(self.warning_min_samples, self.critical_min_samples, self.recovery_min_samples) < 1:
...
class CrowdResult:
    ...
    candidate_level: CrowdLevel | None
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        out["candidate_level"] = self.candidate_level.value if self.candidate_level else None
        return out
...
class _AlertStateMachine:
    ...
    def update(
        self, target: CrowdLevel, timestamp: float
    ) -> tuple[CrowdLevel, bool, bool, CrowdLevel | None, int, float]:
        ...
        changed = False
        candidate_level: CrowdLevel | None = None
        candidate_samples = 0
        ...
        if target == self.level:
            ...
            self._candidate_since = None
            self._candidate_samples = 0
        elif self.level == CrowdLevel.CRITICAL and target != CrowdLevel.NORMAL:
            # A warning sample is not proof that a critical crowd has cleared.
            self._candidate = None
            self._candidate_since = None
            self._candidate_samples = 0
        else:
            ...
            candidate_duration = max(0.0, timestamp - since)
            if candidate_duration >= required and candidate_samples >= self._required_samples(target):
                old = self.level
                self.level = target
                self._candidate = None
                self._candidate_since = None
    ...
    def _required_samples(self, target: CrowdLevel) -> int:
        if _LEVEL_RANK[target] < _LEVEL_RANK[self.level]:
            return self.config.recovery_min_samples
        if target == CrowdLevel.CRITICAL:
            return self.config.critical_min_samples
        return self.config.warning_min_samples
...
class _FlowCounter:
    def __init__(self, config: CrowdConfig) -> None:
        ...
        self._states: dict[tuple[str, str], tuple[int, float, float, Point]] = {}
        ...
        for item in points:
            ...
            for line in self.config.flow_lines:
                ...
                if previous is not None:
                    previous_side, last_cross, _, previous_point = previous
                    if (
                        side != previous_side
                        and timestamp - last_cross >= self.config.line_cross_cooldown_s
                        and _segments_intersect(previous_point, item.image, p1, p2)
                    ):
                    ...
                    self._states[key] = (side, last_cross, timestamp, item.image)
                else:
                    self._states[key] = (side, -math.inf, timestamp, item.image)

        cutoff = timestamp - self.config.track_ttl_s
        stale = [key for key, (_, _, last_seen, _) in self._states.items() if last_seen < cutoff]
        for key in stale:
...
class CrowdAnalyzer:
    ...
    def update(
        ...
        *,
        ...
        level, changed, notify, candidate_level, candidate_samples, candidate_duration = self._alerts.update(
...
def _segments_intersect(first_a: Point, first_b: Point, second_a: Point, second_b: Point) -> bool:
    ...
    if o1 * o2 < -eps and o3 * o4 < -eps:
        return True
    return (
        (abs(o1) <= eps and _point_on_segment(second_a, first_a, first_b))
        or (abs(o2) <= eps and _point_on_segment(second_b, first_a, first_b))
        or (abs(o3) <= eps and _point_on_segment(first_a, second_a, second_b))
        or (abs(o4) <= eps and _point_on_segment(first_b, second_a, second_b))
    )
