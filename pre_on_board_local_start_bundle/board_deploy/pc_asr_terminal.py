...
def main() -> int:
    ...
    print("=" * 60, flush=True)
    print(f"[ASR-TERMINAL] 监听 {args.host}:{args.port}", flush=True)
    print("[ASR-TERMINAL] 等待板端连接…（请确认板端 ASR 已启动且 BOARD_RESULT_HOST 指向本机）", flush=True)
        ...
        try:
            ...
            while True:
                ...
                if msg_type == "asr_partial":
                    ...
                    log_jsonl(args.log_jsonl, {"t": time.perf_counter(), "type": "partial", "text": text})
                    ...
                    log_jsonl(args.log_jsonl, {"t": time.perf_counter(), "type": "final", "text": board_text})
                    ...
                    text = str(msg.get("text", ""))
                    print_line("最终>>", text)
                    log_jsonl(args.log_jsonl, {"t": time.perf_counter(), "type": "final", "text": text})
        finally:
            try:
                conn.close()
            except OSError:
