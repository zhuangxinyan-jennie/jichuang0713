"""TBE compile inside PassContext."""
import paramiko
...
python3 <<'PY'
import json, traceback, struct, sys, glob
...
sftp = ssh.open_sftp()
with sftp.open("/tmp/probe_passctx.sh", "w") as f:
    f.write(script.replace("\r\n", "\n"))
sftp.close()
_, stdout, _ = ssh.exec_command("/bin/bash /tmp/probe_passctx.sh 2>&1", timeout=900)
