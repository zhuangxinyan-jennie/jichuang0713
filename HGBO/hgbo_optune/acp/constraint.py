...
def check_valid_config(
    ...
    if static_config.get("operator") == "PoseLetterbox720p":
        if config.get("pipeline_mode") == "staged" and block_dim != 1:
            reasons.append("PoseLetterbox720p staged pipeline requires blockDim=1")
        if config.get("pipeline_mode") == "staged" and int(config.get("buffer_num", 1)) != 2:
            reasons.append("PoseLetterbox720p staged pipeline requires buffer_num=2")
